...
// scale to window size/
  window.onresize = function() {
    __.width=document.getElementsByClassName("parcoords")[0].offsetWidth;//TODO: Improve this - Doesn't work with multiple graphs
    pc.resize();
    pc.brushReset();
  };
...
pc.autoscale = function() {
  ...
  var defaultScales = {
    ...
    "quality": function(k) {
      return d3.scale.ordinal()
        .domain(['Mala','Pobre','Aceptable','Buena','Excelente'])
        .rangePoints([h()+1, 1])
    }
  };
  ...
  ctx.foreground.lineWidth = 4; //JGarcia Changed Original lineWidrh 1.4
...
function selected() {
  ...
  var within = {
    ...
    "price": function(d,p,dimension) {
    return extents[dimension][0] <= d[p] && d[p] <= extents[dimension][1]
    },
    "string": function(d,p,dimension) {
      return extents[dimension][0] <= yscale[p](d[p]) && yscale[p](d[p]) <= extents[dimension][1]
    },
    "quality": function(d,p,dimension) {
    return extents[dimension][0] <= yscale[p](d[p]) && yscale[p](d[p]) <= extents[dimension][1]
  }
  };
